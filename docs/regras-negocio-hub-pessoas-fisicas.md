...
| 19 | Dependência técnica | Rede/credenciais para `rh.funcionarios` | Adapters de pessoa precisam ler/escrever na base do onsafety-epi | Hoje os adapters de empresa só têm acesso configurado ao Supabase n8n, não ao Supabase do onsafety-epi | — | Bloqueante — ver dúvida global 1 |
| 20 | Dependência técnica | RLS por perfil em `rh.funcionarios` | Adapter tenta gravar via `anon` key, padrão usado no hub-empresas | Padrão atual do hub bypassa RLS via grants diretos; `rh.funcionarios` tem dado sensível (bancário, documentos) sob RLS | — | Bloqueante — ver dúvida global 2 |
...
1. **Topologia de rede/credenciais**: `rh.funcionarios` vive no Supabase do onsafety-epi; o hub-empresas atual só fala com o Supabase n8n. Precisa decisão: adapters de pessoa acessam o Supabase do onsafety-epi diretamente, ou há uma réplica intermediária?
