          ...
          <table className="w-full text-[13px] border-collapse">
            ...
            <tbody>
              {colaboradores?.map((col, i) => (
                ...
                >
                  <Td><span style={{ fontWeight: 600, color: "var(--ink)" }}>{col.nome}</span></Td>
                  ...
                  <Td align="right">
                    <SeloBotao
                      ...
                      action={async () => { "use server"; await alternarAtivo(col.id, col.ativo); }}
                    >
...
function Campo({
  label, name, placeholder, type = "text", mono = false, className = "",
}: {
  label: string; name: string; placeholder?: string; type?: string; mono?: boolean; className?: string;
}) {
  return (
    <label className={"flex flex-col gap-1.5 " + className}>
      <span className="text-[11px] font-medium uppercase tracking-[0.08em]" style={{ color: "var(--ink-tertiary)" }}>{label}</span>
      <input
...
function Th({ children, align = "left" }: { children: React.ReactNode; align?: "left" | "right" }) {
  return (
    <th className={"px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.1em] " + (align === "right" ? "text-right" : "text-left")} style={{ color: "var(--ink-tertiary)" }}>
      {children}
    </th>
  );
}
