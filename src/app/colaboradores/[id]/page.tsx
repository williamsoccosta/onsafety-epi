...
export default async function FichaColaborador({
  ...
  const { data: entregas } = await supabase
    .schema("epi")
    .from("movimentacoes")
    .select("id,criado_em,quantidade,observacao,epi_id(nome,complemento,ca,ca_validade),obra_id(nome)")
    .eq("colaborador_id", id)
    .eq("motivo", "Entrega")
    .order("criado_em", { ascending: false });

  const totalItens = (entregas ?? []).reduce((sum, e) => sum + Math.abs(e.quantidade as number), 0);

  return (
    <>
      {/* Acao de topo — oculta na impressao */}
      <div
        className="flex items-center justify-between px-8 py-4 border-b print:hidden"
      ...
      {/* Ficha — visivel na tela e na impressao */}
      <main className="px-8 py-8 max-w-4xl mx-auto print:px-0 print:py-0 print:max-w-none">
        {/* Cabecalho da ficha */}
        <header className="mb-8 print:mb-6">
          <div
            className="rounded-lg border p-6 print:rounded-none print:border-0 print:border-b print:pb-6"
            style={{ borderColor: "var(--line)", background: "var(--surface)" }}
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="mt-3 flex flex-wrap gap-x-6 gap-y-1">
                  ...
                  <Campo
                    rotulo="Situacao"
                    valor={colab.ativo ? "Ativo" : "Inativo"}
                    destaque={colab.ativo ? "ok" : "off"}
                  />
                </div>
              </div>
              <div
                className="shrink-0 rounded-md border px-4 py-3 text-center print:border"
              ...
              <tbody>
                {(entregas ?? []).map((e, i) => {
                  const epi = e.epi_id as unknown as {
                    nome: string; complemento: string | null; ca: string | null; ca_validade: string | null;
                  };
                  const obra = e.obra_id as unknown as { nome: string };
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        ...
                        {epi.complemento && (
                          <span className="block text-[11px]" style={{ color: "var(--ink-tertiary)" }}>{epi.complemento}</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        {epi.ca ? (
                      ...
                      <td className="px-4 py-3 text-center">
                        <div
                          className="inline-block w-28 border-b-2 h-8"
        ...
        <footer className="mt-10 pt-6 border-t space-y-3" style={{ borderColor: "var(--line)" }}>
          ...
          <div className="flex gap-16 pt-4">
            <AssinaturaBloco rotulo="Assinatura do Colaborador" />
            <AssinaturaBloco rotulo="Responsavel pelo registro" />
          </div>
...
function Campo({
  ...
  return (
    <div>
      <span className="text-[10px] uppercase tracking-[0.1em]" style={{ color: "var(--ink-muted)" }}>
        {rotulo}{" "}
      </span>
      <span
        className={"text-[13px] font-medium " + (mono ? "tabular" : "")}
        style={{ color: destaque === "ok" ? "var(--success)" : destaque === "off" ? "var(--ink-tertiary)" : "var(--ink-secondary)" }}
      >
        {valor}
      </span>
    </div>
...
function Th({ children, align = "left" }: { children: React.ReactNode; align?: "left" | "right" | "center" }) {
  ...
  return (
    <th className={"px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.1em] " + cls} style={{ color: "var(--ink-tertiary)" }}>
      {children}
    </th>
  );
}

function AssinaturaBloco({ rotulo }: { rotulo: string }) {
  return (
    <div className="flex-1">
      <div className="border-b-2 h-12 mb-2" style={{ borderColor: "var(--line-strong)" }} />
      <p className="text-[11px]" style={{ color: "var(--ink-tertiary)" }}>{rotulo}</p>
    </div>
  );
}
